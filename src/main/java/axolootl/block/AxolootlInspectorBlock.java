...
public class AxolootlInspectorBlock extends WaterloggedHorizontalBlock implements EntityBlock {

    public AxolootlInspectorBlock(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResult use(BlockState pState, Level pLevel, BlockPos pPos, Player pPlayer, InteractionHand pHand, BlockHitResult pHit) {
        if (pLevel.isClientSide()) {
            ...
            if (pPlayer instanceof ServerPlayer serverPlayer && pLevel.getBlockEntity(pPos) instanceof AxolootlInspectorBlockEntity blockEntity) {
                // validate controller
                ...
                if (blockEntity.isMenuAvailable(serverPlayer, blockEntity.getController().orElse(null))) {
                    ...
                    NetworkHooks.openScreen(serverPlayer, blockEntity, AxRegistry.MenuReg.writeControllerMenu(controllerPos, pPos, AxRegistry.AquariumTabsReg.AXOLOOTL_INSPECTOR.get().getSortedIndex(), -1));
                }
            }
